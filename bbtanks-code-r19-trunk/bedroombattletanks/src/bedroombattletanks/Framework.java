...
public class Framework extends JGEngine {
	...
	public void initCanvas() {
		...
		setCanvasSettings(64, // width of the canvas in tiles
	...
	/**
	 * This method is called when the engine has finished initialising and is ready
	 * to produce frames. Note that the engine logic runs in its own thread,
	 * separate from the AWT event thread, which is used for painting only. This
	 * method is the first to be called from within its thread. During this method,
	 * the game window shows the intro screen.
	 */
	...
	public void doFrameInGame() {
		...
		if (!gameInfo.gameLost) {
			if (powerUpCountDown <= 0) {
				...
				if (newPowerUpNo > 0) {
					new WeaponBox((int) random(0, pfWidth()), (int) random(0, pfHeight()), gameInfo, "fireball-box",
							"HomingMissile");
				}
				if (newPowerUpNo > 0) {
					new WeaponBox((int) random(0, pfWidth()), (int) random(0, pfHeight()), gameInfo, "grenade-box",
							"Grenade");
				}

				// else if (newPowerUpNo > 0) {
				// new
				// MuddleBox("MuddleBox",(int)random(0,pfWidth()),(int)random(0,pfHeight()));
				// }
	...
	/**
	 * Any graphics drawing beside objects or tiles should be done here. Usually,
	 * only status / HUD information needs to be drawn here.
	 */
	...
	public void paintFrameGameOver() {
		...
		drawString("All " + gameInfo.loser + "'s base are belong to " + gameInfo.winner + ".", viewWidth() / 2,
				viewHeight() / 2, 0);
