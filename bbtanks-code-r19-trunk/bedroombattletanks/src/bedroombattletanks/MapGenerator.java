...
public class MapGenerator {
	...
	MapGenerator(GameInfo gameInfo, JGEngine eng) {
		...
		for (int i = 1; i <= eng.random(1, 2); i++) {
			...
			case 1:
				placeObject(new Bed(0, 0, 'H', gameInfo));
			case 2:
				placeObject(new Bed(0, 0, 'V', gameInfo));
			}
		}
		for (int i = 1; i <= eng.random(1, 2); i++) {
			switch ((int) eng.random(1, 2)) {
			case 1:
				placeObject(new Chair(0, 0, 'H', gameInfo));
			case 2:
				placeObject(new Chair(0, 0, 'V', gameInfo));
			}
		}
		for (int i = 1; i <= eng.random(1, 15); i++) {
			switch ((int) eng.random(1, 2)) {
			case 1:
				placeObject(new Pencil(0, 0, 'H', gameInfo));
			case 2:
				placeObject(new Pencil(0, 0, 'V', gameInfo));
			}
		}
		printGrid(objectMap);
	}
